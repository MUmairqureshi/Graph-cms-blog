...
interface Porps {
  ...
  const PostDetail : FC<Porps>= ({post}) => {
  ...
  switch(type){
    case'heading-three':
    return <h3 key={index} className='mb-text-xl font-semibold mb-4 '>{modifiedText.map((item : string, i: number) => <React.Fragment key={i}>{item}</React.Fragment>)}</h3>
    case 'paragraph':
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg lg:p-8 pb-12 mb-8">
      ...
      <div className="hidden   md:flex lg:mb-0  mr-8 items-center">
           
           <img
        
               className="align-middle rounded-full h-[40px] w-[40px]"
